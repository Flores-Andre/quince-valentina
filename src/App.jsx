...
import sobreImg from './assets/sobre.webp'; // <-- Tu sobre agregado aquí
import sobreAbiertoImg from './assets/sobre-abierto.jpg'; /* o .png según tu archivo */
...
function App() {
  ...
  /* 👇 1. Creamos la referencia para el audio 👇 */
  const audioRef = useRef(null);

  /* 👇 2. Esta es la nueva función que hará la magia 👇 */
  const handleAbrirInvitacion = () => {
    setIsOpened(true); // Cambia la pantalla a "abierta"
    if (audioRef.current) {
      audioRef.current.play(); // ¡Le da Play a la música!
    }
  };
...
}, [isOpened]); // 👈 el cambio clave: ahora depende de isOpened

  const formatEventDate = () => {
    const options = { day: 'numeric', month: 'long', year: 'numeric' };
    return EVENT_DATE.toLocaleDateString('es-ES', options);
  };
  ...
  >
    <path d="M12 2L2 9l10 13L22 9 12 2zm0 2.5L18.5 9H14l-2-4.5zM12 4.5L9.5 9H5.5l6.5-4.5zM5.29 10.5H9.7l1.8 8.1-6.21-8.1zm7.5 8.1l1.8-8.1h4.41l-6.21 8.1z"/>
        ...
        <div className="message-card">
          <p className="reveal">
            "Dios y la vida nos han bendecido viendo crecer a nuestro hija, convertida hoy en una joven maravillosa. Al
            llegar a sus 15 años, queremos compartir nuestra felicidad con quienes han sido parte de su camino. ¡Acompáñenos a festejar 
            este gran día!" 
          </p>
        </div>
      </section>
    ...
    <>
      ...
      <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
        ...
        <section className="rsvp">
          ...
          <p className="rsvp-sub">Por favor confirmar antes del 20/09/2026</p>
          <RSVPForm />
        </section>
